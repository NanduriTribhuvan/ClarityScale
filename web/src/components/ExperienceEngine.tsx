...
/**
 * ExperienceEngine — the "Living Growth Engine" client island.
 *
 * A superset of the baseline animation island used on the flagship homepage.
 * It loads GSAP + ScrollTrigger + SplitText from npm (after first paint) and
 * layers a cinematic experience onto the pre-rendered DOM:
 *   • SplitText kinetic hero headline
 *   • scroll-scrub hero depth parallax
 *   • magnetic cursor / buttons / card tilt
 *   • scroll reveals, section staggers, counters, dashboard build
 *
 * Robustness:
 *   • Every feature runs inside `safe()` so one failure can't abort the rest.
 *   • prefers-reduced-motion → functional behaviour only, motion suppressed.
 *   • If GSAP fails to load → IntersectionObserver reveals all content.
 */

/* eslint-disable @typescript-eslint/no-explicit-any */

/** Run an initializer in isolation; never let one feature break the others. */
function safe(fn: () => void) {
  try {
    fn();
  } catch (err) {
    console.warn('[experience] feature init failed:', err);
  }
}
...
export default function ExperienceEngine() {
  useEffect(() => {
    ...
    safe(initMobileMenu);
    safe(initPricingToggle);
    safe(initActiveNavLinks);
    safe(initMobileBottomNav);
    safe(initNavbarScrollState);
    ...
    (async () => {
      ...
      try {
        ...
        try {
          ...
          SplitText = null; // kinetic headline becomes a graceful fade
        }
        if (cancelled) return;
        ...
        safe(() => initNavbarEntrance(gsap));
        safe(() => initKineticHero(gsap, SplitText));
        safe(() => initHeroAnimations(gsap));
        safe(() => initHeroScrollParallax(gsap, ScrollTrigger));
        safe(() => animateCounters(gsap));
        safe(() => initScrollReveal(gsap, ScrollTrigger));
        safe(() => initSectionStaggers(gsap, ScrollTrigger));
        safe(() => initParallax(gsap));
        safe(() => initFloatCards(gsap));
        safe(() => initDashboardAnimations(gsap, ScrollTrigger));
        safe(() => initFeatureTilt());
        safe(() => initMagneticButtons());
        safe(() => initHeroShimmer(gsap));
        safe(() => initTrustMarquee(gsap));
        safe(() => initCustomCursor(gsap));
        safe(() => initProgressBeam(gsap, ScrollTrigger));
        safe(() => initVisibilityPause(gsap));

        // Re-measure after fonts/layout settle so pinned/scrub triggers align.
...
/* ============================================================ KINETIC HERO */
function initKineticHero(gsap: any, SplitText: any) {
  const headline = document.querySelector<HTMLElement>('.hero-headline');
  if (!headline) return;

  if (!SplitText) {
    // Fallback: a simple lift+fade if SplitText is unavailable.
    gsap.from(headline, { y: 40, opacity: 0, duration: 0.8, ease: 'power3.out', delay: 0.35 });
    return;
  }
  ...
  gsap.from(split.chars, {
    ...
    ease: 'power4.out',
    delay: 0.35,
    // Restore original markup (keeps the gradient-text span intact) when done.
    onComplete: () => split.revert(),
  });
}

/* ============================================================ HERO ENTRANCE */
// Note: the headline is handled by initKineticHero, so it's omitted here.
function initHeroAnimations(gsap: any) {
  const tl = gsap.timeline({ delay: 0.3 });
  tl.from('.hero-badge', { y: 20, opacity: 0, duration: 0.6, ease: 'power3.out' });
  tl.from('.hero-sub', { y: 20, opacity: 0, duration: 0.6, ease: 'power2.out' }, '-=0.2');
  tl.from(
    '.hero-cta-row .btn',
  ...
  tl.from('.hero-logo-wrap', { scale: 0.85, opacity: 0, duration: 0.9, ease: 'back.out(1.4)' }, '-=0.7');
  tl.from(
    '.float-card',
...
function initHeroScrollParallax(gsap: any, _ScrollTrigger: any) {
  const hero = document.querySelector('.hero');
  if (!hero) return;
  const common = { trigger: hero, start: 'top top', end: 'bottom top', scrub: true } as const;
  const logo = document.querySelector('.hero-logo-wrap');
  if (logo) gsap.to(logo, { yPercent: 28, ease: 'none', scrollTrigger: common });
  const content = document.querySelector('.hero-content');
  if (content) gsap.to(content, { yPercent: 14, opacity: 0.55, ease: 'none', scrollTrigger: common });
  gsap.to('.hero .orb', { yPercent: 40, ease: 'none', scrollTrigger: common });
}
...
function initSectionStaggers(gsap: any, ScrollTrigger: any) {
  const stagger = (trigger: string, target: string, opts: Record<string, unknown> = {}) => {
...
function initCustomCursor(gsap: any) {
  ...
  document.querySelectorAll('a, button, .btn, input, textarea, .ib-tile').forEach((el) => {
...
function initProgressBeam(gsap: any, ScrollTrigger: any) {
  ...
  if (!beam) return;
  gsap.to(beam, { scaleX: 1, ease: 'none', scrollTrigger: { start: 0, end: 'max', scrub: 0.3 } });
  ScrollTrigger.refresh();
}
